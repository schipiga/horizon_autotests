...
def test_edit_volume(volume, volumes_steps):
    old_name, volume.name = volume.name, volume.name + ' (updated)'
    volumes_steps.edit_volume(name=old_name, new_name=volume.name)


@pytest.mark.parametrize('volumes_count', [1, 2])
def test_delete_volumes(volumes_count, volumes_steps, create_volumes):
